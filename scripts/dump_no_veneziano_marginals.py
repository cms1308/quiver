...
def _bound_rhs_only(nf_bound_str: str) -> str:
    """Extract just the RHS expression: 'N_f ≤ 2*N - 1' → '2N-1'."""
    if not nf_bound_str:
        return "-"
    import re as _re
    s = nf_bound_str.strip()
    s = s.replace("≤", "").replace("N_f", "").replace(" ", "")
    s = s.replace("*N", "N")
    s = _re.sub(r"\b1N\b", "N", s)
    s = s.lstrip()
    return s if s else "-"
...
def _node_bound_segment(nf_bound_str: str, comparison: str) -> str:
    """Per-node bound segment in TeX (without surrounding $).

    If the node is already at b_0 = 0 with anomaly-only matter, the bound is
    'N_f = 0' (no extra fundamentals possible). Otherwise use the chosen
    comparison (\\le or <) with the bound RHS."""
    if not nf_bound_str:
        return "-"
    if "conformal" in nf_bound_str:
        return "N_f = 0"
    return f"N_f {comparison} {_bound_rhs_only(nf_bound_str)}"
...
def main() -> None:
    ...
    rows = list(con.execute(
        "SELECT * FROM theory "
        "WHERE gauge_pair='SU-SU' AND veneziano=0 AND class_id IS NOT NULL "
        "AND rank0_mult=1 AND rank1_mult=1 "
        "ORDER BY class_id, theory_id"
    ))
    print(f"Processing {len(rows)} SU(N)-SU(N) (1,1) no-Veneziano theories...", file=sys.stderr)

    OUT.parent.mkdir(parents=True, exist_ok=True)
    with OUT.open("w") as f:
        f.write("% Auto-generated by scripts/dump_no_veneziano_marginals.py\n")
        f.write("% Requires: \\usepackage{booktabs,longtable,amssymb,amsmath,pdflscape}\n")
