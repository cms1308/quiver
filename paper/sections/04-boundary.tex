...
node.  Class~877 is omitted because its empty-node morphology places
the entire matter sector on bifundamentals (all surviving R-charges
vanish at leading $N$, as in class~1); the sub-quiver decoupling
limit required for $\alpha_{b}^{(a\star)}$ therefore needs a separate
treatment (Section~\ref{sec:class-special}).  The
...
\begin{center}
  \begin{tabular}{lll}
    ...
    Marginal: $\alpha=0$ at both boundaries
          & $(0,0)$      & $\{44\}$ \\
    Degenerate: leading-$N$ $a$-max undefined
          & ---          & $\{877\}$ \\
    \bottomrule
  \end{tabular}
\end{center}
