...
\begin{table}[h]
  ...
  \caption{The nine non-Veneziano $\SU(N)\times\SU(N)$ universality
    ...
    enumeration finds in the class.  For all eight classes with a
    defined $a/N^{2}$ the leading-$N$ $a=c$ relation holds.  Class~877
    has one node with empty intrinsic matter content, so that
    $a$-maximisation of the full two-node theory has no interior
    critical point and $a/N^{2}$ is not defined at leading $N$
...
\begin{table}[h]
  ...
  \begin{tabular}{clc}
    ...
    $877$& not defined ($a$-maximum diverges; see Section~\ref{sec:class-special})                           & --- \\
    \bottomrule
  \end{tabular}
  \caption{Exact R-charges at the $a$-maximal critical point.  Class
    ...
    (ditto).  Class~877 has one node with empty intrinsic matter and
    no interior $a$-maximum (Section~\ref{sec:class-special}).  For
...
\paragraph{Class 877 ($a/N^{2}$ undefined; empty-node class).}
...
node.  The $a$-maximisation of the full two-node theory then has a
flat direction: the R-charge of the empty node's gauginos is
unconstrained except by the anomaly-free condition at the other
node, and the symbolic trial $a$-function does not admit a finite
interior critical point with negative-definite Hessian.  The leading
$N$ central charge $a/N^{2}$ is therefore not defined for class 877,
and the boundary coefficients $b_{0}^{(a)}(g_{b}^{\star})$ cannot be
computed from the leading-$N$ prescription used for the other
classes.  We retain class 877 in the classification for completeness
— it is a legitimate solution of the enumeration constraints
(\ref{eq:AF-lead}), (\ref{eq:anomaly-N}),
(\ref{eq:anomaly-const}) — but flag it as a degenerate case whose
RG topology requires an analysis beyond the scope of this paper.
...
boundary at leading $N$; class $877$ has a degenerate
$a$-maximisation with no interior critical point.  The eight classes
with a defined leading-$N$ sign pattern $(1,2,4,9,16,21,23,44)$ are
retained in the RG-flow topology analysis of
Sections~\ref{sec:boundary}--\ref{sec:phases}; class~877 is omitted
from the boundary-coefficient tables.
