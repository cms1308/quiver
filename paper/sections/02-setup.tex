...
\paragraph{The range of $k_{\text{tot}}$.}
The leading-$N$ AF bound \eqref{eq:AF-lead} with
$n_{\adj}^{(a)}=n_{S}^{(a)}=n_{\bar S}^{(a)}=n_{A}^{(a)}=n_{\bar A}^{(a)}=0$
(pure bifundamental quiver, no intra-node matter) admits
\begin{equation}
  k_{\text{tot}}\;\le\;6\,,
  \label{eq:k-strict}
\end{equation}
saturating at $k_{\text{tot}}=6$ with $b_{0}^{(a)}=0$
\emph{exactly} (marginal at one loop for all $N$).  The enumeration
in \texttt{quiver\_generation.py:\_enumerate\_edges} uses
\texttt{max\_multiedge=2} per edge type and loops over all four
$\SU$--$\SU$ edge types $\{++,--,+-_{\text{fwd}},+-_{\text{bwd}}\}$,
so in principle up to $k_{\text{tot}}=8$; the exact-anomaly
condition \eqref{eq:anomaly-N-nonVen}, together with the leading-$N$
AF inequality \eqref{eq:AF-lead}, restricts the physically
realised values in the non-Veneziano equal-rank sector to
\begin{equation}
  k_{\text{tot}}\;\in\;\{1,2,3,4,6\}\,.
  \label{eq:k-values}
\end{equation}
The absence of $k_{\text{tot}}=5$ is a consequence of the anomaly
identity together with the tensor content: with no tensors at all
(no $S,\bar S,A,\bar A,\adj$), the $N$-coefficient
condition~\eqref{eq:anomaly-N-nonVen} reduces to $\deg^{+}_{a}
-\deg^{-}_{a}=0$ at each node, which requires $k_{\text{tot}}$ to be
even; the only odd $k_{\text{tot}}$ values realised by the enumeration
are $\{1,3\}$, both relying on a non-zero tensor asymmetry
$(n_{S}-n_{\bar S})+(n_{A}-n_{\bar A})\ne 0$ to close the anomaly,
and at $k_{\text{tot}}=5$ the tensor content demanded by the AF bound
\eqref{eq:AF-lead} is incompatible with this asymmetry.  The
unique class supporting $k_{\text{tot}}=6$ is the purely
bifundamental class 44 (no tensor matter), which is leading-$N$
marginal.
...
the non-Veneziano limit, where \emph{nine} universality classes arise
