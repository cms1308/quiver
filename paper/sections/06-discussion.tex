...
boundaries; classes 1 and 9 formally sit in the Figure~5 sign pattern
but have interior R-charges below the unitarity bound (at $R=0$ and
$R=1/2$ respectively), so are non-interacting at leading $N$; and
class 877 is the empty-node case in which one node carries no
intrinsic matter, for which the 9 representatives surviving the
leading-$N$ flat-direction exclusion admit a unique interior
$a$-maximum at $a/N^{2}=0$ with all surviving matter R-charges
vanishing (qualitatively matching class~1 but morphologically
distinct); its boundary analysis requires a separate treatment
because the sub-quiver decoupling limit is subtle for an all-bifundamental
matter sector.
...
\begin{itemize}
  ...
  \item \textbf{Sub-leading treatment of classes 1, 9, 44, and 877.}
    ...
    bound ($R=0$ and $R=1/2$ respectively); lifting the R-charges
    into the interior of the polytope by sub-leading corrections (or
    finding a different R-symmetry extremum) would give their
    physical IR.  Class 877 has one node with empty intrinsic
    matter and a leading-$N$ $a/N^{2}=0$ with all surviving matter
    R-charges vanishing (qualitatively matching class~1); its
    boundary-coefficient analysis is complicated by the fact that the
    empty node's entire matter sector consists of bifundamentals, so
    a separate sub-quiver decoupling treatment is required.
