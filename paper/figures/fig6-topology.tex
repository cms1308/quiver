...
\begin{figure}[h]
  ...
  \begin{tikzpicture}[
    ...
    % axes
    \draw[->, thick] (-0.3,0) -- (5.4,0) node[right] {$g_{1}$};
    \draw[->, thick] (0,-0.3) -- (0,4.7) node[above] {$g_{2}$};
    % fixed points
    \node[fpUV, label={below left:UV: $(0,0)$}] (UV) at (0,0) {};
    \node[fpIR, label={[align=right]left:{\textbf{axis SCFT 2}\\$(0,\,g_{2}^{\star})$}}]
          (A1) at (0,3.4) {};
    \node[fpIR, label={[align=center]below:{\textbf{axis SCFT 1}\\$(g_{1}^{\star},\,0)$}}]
          (A0) at (4.2,0) {};
    ...
    % labels
    \node[red!70!black]  at (1.1, 3.60) {\small $\alpha_{2}<0$};
    \node[red!70!black]  at (4.20, 0.50) {\small $\alpha_{1}<0$};
  \end{tikzpicture}
  \caption{Figure~6 topology: $(\alpha_{1},\alpha_{2})=(-,-)$.  Both
    axis SCFTs (filled red circles) are IR-stable; a separatrix
    (dashed violet) through the interior saddle $C$ divides the
    basins of attraction.  Realised by class~$16$ deformed by $\Nf$
    fundamentals on node~$2$ in the window
    $0.427\,N-1<\Nf<N-1$.}
  \label{fig:fig6}
\end{figure}
