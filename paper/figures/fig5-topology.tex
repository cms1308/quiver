...
\begin{figure}[h]
  ...
  \begin{tikzpicture}[
    ...
    % axes
    \draw[->, thick] (-0.3,0) -- (5.2,0) node[right] {$g_{0}$};
    \draw[->, thick] (0,-0.3) -- (0,4.5) node[above] {$g_{1}$};
    % fixed points
    \node[fpUV, label={below left:UV: $(0,0)$}] (UV) at (0,0) {};
    \node[fp,   label={left:$(0,\,g_{1}^{\star})$}]      (A1) at (0,3.2) {};
    \node[fp,   label={below:$(g_{0}^{\star},\,0)$}]     (A0) at (4.0,0) {};
    \node[fpIR, label={[align=left]above right:{\textbf{IR SCFT $C$}\\$(g_{0}^{\star\star},g_{1}^{\star\star})$}}]
          (IR) at (2.8,2.2) {};
    % flows from UV
    ...
    % labels
    \node[red!70!black]  at (0.95, 3.40) {\small $\Bcond_{A}<0$};
    \node[red!70!black]  at (4.05, 0.48) {\small $\Bcond_{B}<0$};
  \end{tikzpicture}
  \caption{Topology (i): $\Bcond_{A},\Bcond_{B}<0$.  Both axes release
    the flow into the interior, and the two-coupling SCFT $C$ at
    $(g_{0}^{\star\star},g_{1}^{\star\star})$ is the global IR
