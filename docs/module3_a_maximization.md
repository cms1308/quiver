...
### `a_maximization.py` — Numerical a-maximization at fixed (N, N_f)

**Data structures:**
- `ChiralField(label, R_index, dim, T_contributions)` — one type of chiral multiplet. `dim` = total dimension (product of gauge rep dims × multiplicity). `T_contributions` = dict mapping node index → effective Dynkin index.
- `AMaxResult(fields, R_charges, a, c, unitarity_ok)` — result container.

**Key functions:**
- `build_fields(quiver, N, N_f) → list[ChiralField]` — expands a quiver at specific (N, N_f) into chiral fields from three sources: rank-2/adj node matter, fund-like matter (from N_f and chiral excess $\delta$), and bifundamental edges.
- `anomaly_matrix(fields, quiver, N) → (A, b)` — linear system $A \cdot R = b$ from $T(\text{adj}_a) + \sum_i T_a(r_i)(R_i - 1) = 0$.
- `a_maximize(quiver, N, N_f) → AMaxResult` — particular solution via `lstsq`, null space via `scipy.linalg.null_space`, maximize $-a_\text{trial}$ over free parameters using BFGS.
- `a_maximize_with_decoupling(quiver, N, N_f) → AMaxResult` — iterative decoupling: when a gauge-invariant operator $\mathcal{O}$ has $R[\mathcal{O}] < 2/3$, adds the linear constraint $\sum R_i = 2/3$ and re-maximizes until all operators satisfy unitarity.
- `gauge_invariant_ops(fields, quiver, R_values) → dict` — enumerates mesons, bilinears ($\text{Tr}\Phi^2$, $S\bar{S}$, $A\bar{A}$, $V \cdot V$, $f \Omega f$), and bifundamental pair operators.

### `a_maximization_large_N.py` — Exact symbolic large-N a-maximization

At large N, all quantities scale polynomially: $T \sim N$, $\dim \sim N^2$. Dividing through gives N-independent leading coefficients. The anomaly constraints become linear in R-charges; the a-function is cubic in the free parameters, so critical points satisfy quadratic equations — yielding exact algebraic R-charges.

**Leading-order coefficient functions:**
- `dim_group_lead(gauge_type)` — $\lim_{N\to\infty} \dim(G)/N^2$: SU→1, SO→1/2, Sp→2
- `T_adj_lead(gauge_type)` — always 1
- `T_rep_lead(gauge_type, rep)` — fund/V: 0 (subleading); adj: 1; rank-2 SU: 1/2; rank-2 SO/Sp: 1
- `dim_rep_lead(gauge_type, rep)` — fund/V: 0; adj SU: 1, SO: 1/2, Sp: 2; rank-2 SU: 1/2, SO: 1/2, Sp: 2
- `T_bifund_lead(ga, gb)`, `dim_bifund_lead(ga, gb)` — per gauge pair

**Key insight:** at large N, all rank-2 representations (S, $\bar{S}$, A, $\bar{A}$) of SU(N) are indistinguishable (same $T_\text{lead} = 1/2$, $\dim_\text{lead} = 1/2$). This produces **universality classes**: theories with different rank-2 content but identical leading-order field structure share the same R-charges and $a/N^2$.

**Functions:**
- `build_fields_large_N(quiver) → list[LeadField]` — leading-order fields (rank-2/adj, chiral excess fund-like, bifundamentals). No N or N_f parameter.
- `a_maximize_large_N(quiver) → LargeNResult` — exact symbolic solver using sympy: null space, symbolic differentiation, `solve()` for quadratic critical-point equations.
- `a_maximize_large_N_fast(quiver) → LargeNResult` — numerical solver using numpy/scipy (BFGS with 8 random restarts, ~40ms/quiver). Used for fast scanning before exact computation.
- `a_maximize_large_N_with_decoupling(quiver) → LargeNResult` — iterative unitarity decoupling at leading order.
