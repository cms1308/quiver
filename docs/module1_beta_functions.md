...
### Implementation: `beta_functions.py`

Key functions:
- `T_adj(gauge_type, N)`, `T_rep(gauge_type, rep, N)`, `T_bifund(gauge_a, gauge_b, N)` — Dynkin indices as `Fraction`
- `A_rep(rep, N)` — cubic anomaly coefficient for SU(N) representations
- `compute_b0(node: NodeSpec) → Fraction` — one-loop beta function coefficient at specific N
- `b0_linear(gauge_type, matter, bifund_neighbors) → (alpha, beta)` — coefficients of $b_0(N) = \alpha N + \beta$
- `is_af_all_N(gauge_type, matter, bifund_neighbors) → bool` — True if $b_0 > 0$ for all valid N (large-N classification criterion)
